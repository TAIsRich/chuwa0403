...
10. @Api: @Api is an annotation provided by Swagger, which is a tool used for documenting and testing RESTful APIs. The @Api annotation is used to decorate a class or interface to provide high-level information about the API.
11. @Mock: @Mock is an annotation provided by Mockito, a popular Java mocking framework used for unit testing. The @Mock annotation is used to create a mock object of a class or interface.
12. @ExtendWith: @ExtendWith is an annotation provided by JUnit 5, the latest version of the JUnit testing framework for Java. It is used to register extensions with JUnit test classes or test methods.
13. @Spy: The @Spy annotation is provided by Mockito, a popular Java mocking framework used for unit testing. The @Spy annotation is used to create a spy object, which is a partial mock of a real object.
14. @InjectMocks: The @InjectMocks annotation is provided by Mockito, a popular Java mocking framework used for unit testing. The @InjectMocks annotation is used to automatically inject mock objects into the fields of a test class.
15. @BeforeAll: The @BeforeAll annotation is provided by JUnit 5 and is used to annotate a static method in a test class that should be executed once before all test methods in the class. It is commonly used for performing setup or initialization tasks that are shared among multiple tests.
16. @BeforeEach: The @BeforeEach annotation is commonly used in unit testing frameworks, such as JUnit, to mark a method that should be executed before each test case in a test class. It is used to set up the initial state or perform any necessary setup actions before each individual test runs. 
