...
When you annotate a test class with @DataJpaTest, it will:
1. Configure an in-memory database (H2 by default) or connect to a real database depending on the configuration.
2. Set up the necessary Spring Data JPA infrastructure, including the EntityManager, EntityManagerFactory, and transaction management.
3. Scan for and configure only the relevant Spring beans, such as repositories, entities, and data access components.
4. Rollback database transactions after each test method by default, ensuring a clean state for subsequent tests.
