package org.example.optional_exercise;
...
public class ShoppingCartUtil {
    ...
    public static void checkItemsInCart(Customer customer) {
        Optional<List<Item>> optionalItem = Optional.ofNullable(customer)
                .map(Customer::getShoppingCart)
                .map(ShoppingCart::getItems);
        if (optionalItem.isPresent() && !optionalItem.get().isEmpty()) {
    ...
    public static void printItemsInCart(Customer customer) {
        ...
        Optional<List<Item>> optionalItem = Optional.ofNullable(customer)
                ...
                .map(ShoppingCart::getItems);
        if (optionalItem.isPresent() && !optionalItem.get().isEmpty()) {
            optionalItem.get().forEach(item -> System.out.println(item.getName()));
        }
    }
