...
public class ShoppingCartUtil {
    ...
    public static Optional<String> getFirstItemName(Customer customer) {
        Optional<String> item = Optional.ofNullable(customer)
                .map(Customer::getShoppingCart)
                .map(ShoppingCart::getItems)
                .filter(items -> !items.isEmpty()) // we could use filter to make sure  is not empty
                .map(l->l.get(0))
                .map(Item::getName);
        return item;
    }
    ...
    public static double getTotalPrice(Optional<Customer> customer) {
        /*
        if(customer.map(Customer::getShoppingCart).isPresent()){
            List<Item> items = customer.get().getShoppingCart().getItems();
            return items.stream().map(item -> item.getPrice()).reduce(Double::sum).get();
        }*/

        return customer.map(Customer::getShoppingCart).map(ShoppingCart::getItems).map(items -> items.stream().mapToDouble(Item::getPrice).sum())
                .orElse(0.0);
        // return 0.0;
    }
    ...
    public static String getFirstItemNameWithAlternative(Customer customer) {
        Optional<String> itemName = Optional.ofNullable(customer)
                .map(Customer::getShoppingCart)
                .map(ShoppingCart::getItems)
                .filter(items -> !items.isEmpty())
                .map(l->l.get(0))
                .map(Item::getName);
        return itemName.orElse(getRandomAlternativeItem());
        // return itemName.orElseGet(ShoppingCartAnswer::getRandomAlternativeItem);
        // use lambda for orElseGet
        // return itemName.orElse(ALTERNATIVE_ITEMS.stream().findAny().get());
    }
    ...
    public static String getFirstItemNameOrThrowException(Customer customer) throws EmptyCartException {
        Optional<String> item = Optional.ofNullable(customer)
                .map(Customer::getShoppingCart).map(ShoppingCart::getItems).filter(items -> !items.isEmpty())
                        .map(items -> items.get(0)).map(Item::getName);
        return item.orElseThrow(()->new EmptyCartException("shopping cart is null or not exists"));
    ...
    public static void checkItemsInCart(Customer customer) {
        // write you code
        Optional<List<Item>> optional = Optional.ofNullable(customer).map(Customer::getShoppingCart)
                .map(ShoppingCart::getItems);
    ...
    public static void printItemsInCart(Customer customer) {
        // write your code
        Optional<List<Item>> items = Optional.ofNullable(customer).map(Customer::getShoppingCart)
                .map(ShoppingCart::getItems);
        items.ifPresent(items1 -> {
            if(!items1.isEmpty())
                items1.stream().map(Item::getName).forEach(System.out::println);
        });
    }
