...
public class ShoppingCartUtil {
    ...
    public static Optional<String> getFirstItemName(Customer customer) {
        Optional<String> res = Optional.ofNullable(customer)
                ...
                .map(item -> item.get(0)).map(Item::getName);
//                .stream().map(Item::getName).findFirst().orElse(null);
        return res;
    }
    ...
    public static double getTotalPrice(Optional<Customer> customer) {
        return customer.map(c -> c.getShoppingCart()).map(s -> s.getItems()).filter(items -> !items.isEmpty())
                .map(lists -> lists.stream().mapToDouble(Item::getPrice).sum())
                .orElse(0.0);
    }
    ...
    public static String getFirstItemNameWithAlternative(Customer customer) {
        return Optional.ofNullable(customer)
                ...
                .map(ShoppingCart::getItems)
                .filter(items -> !items.isEmpty())
                .map(item -> item.get(0)).map(Item::getName).orElseGet(() -> "Ranodm");
    }
    ...
    public static String getFirstItemNameOrThrowException(Customer customer) throws EmptyCartException {
        return Optional.ofNullable(customer)
                ...
                .map(ShoppingCart::getItems)
                .filter(items -> !items.isEmpty())
                .map(item -> item.get(0)).map(Item::getName).orElseThrow(() -> new EmptyCartException("Empty"));
    }
    ...
    public static void checkItemsInCart(Customer customer) {
        // write you code
        Optional<List<Item>> itemsOptional = Optional.ofNullable(customer).map(Customer::getShoppingCart).map(ShoppingCart::getItems);
    ...
    public static void printItemsInCart(Customer customer) {
        Optional.ofNullable(customer).map(Customer::getShoppingCart).map(ShoppingCart::getItems)
                .ifPresent((items) -> {
