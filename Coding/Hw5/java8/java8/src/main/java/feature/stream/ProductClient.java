...
public class ProductClient {
    ...
    public void toList() {
        ...
        List<Product> filtered = productList.stream().filter(pro -> pro.getStock() > 10).collect(Collectors.toList());
    ...
    public void toSet() {
        ...
        Set<String> uniqueSet = productList.stream().map(Product::getCategory).collect(Collectors.toSet());
    ...
    public void joining() {
        ...
        String res = productList.stream().map(Product::getName).collect(Collectors.joining(", "));
        ...
        res = productList.stream().map(Product::getName).collect(Collectors.joining("/n"));
    ...
    public void counting() {
        ...
        Map<String, Long> res = productList.stream().collect(Collectors.groupingBy(Product::getCategory, Collectors.counting()));
    ...
    public void summingInt() {
        ...
        int res = productList.stream().collect(Collectors.summingInt(Product::getStock));
    ...
    public void groupingBy() {
        ...
        Map<String, Integer> res = productList.stream().collect(Collectors.groupingBy(Product::getCategory, Collectors.summingInt(Product::getStock)));
        ...
        Map<String, Double> rees = productList.stream().collect(Collectors.groupingBy(Product::getCategory, Collectors.averagingDouble(Product::getPrice)));
    ...
    public void maxBy() {
        ...
        Optional<Product> res = productList.stream().collect(Collectors.maxBy((p1, p2) -> (int)(p1.getPrice() - p2.getPrice())));
        Optional<Product> maxPriceProduct = productList.stream()
                .collect(Collectors.maxBy(Comparator.comparingDouble(Product::getPrice)));
        System.out.println(res + " " + maxPriceProduct);
    }

    @Test
    public void partitioningBy() {
        // 给定一个Product列表，使用Stream API的partitioningBy()方法按产品价格是否高于100分组。
        Map<Boolean, List<Product>> res = productList.stream().collect(Collectors.partitioningBy(p -> p.getPrice() > 100));
    ...
    @Test
    public void mapping() {
        Set<String> res = productList.stream()
                .map(Product::getName)
    ...
    public void findFirst() {
        // findFirst example
        Optional<Product> res = productList.stream().filter(p -> p.getPrice() > 100).findFirst();
    ...
    @Test
    public void findAny() {
        Optional<Product> res = productList.stream().filter(p -> p.getStock() > 100).findAny();
        // findAny example
