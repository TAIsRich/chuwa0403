package c08_future.batch_payments;
...
import static c08_future.batch_payments.OrderProcessUtil.*;
...
public class BatchPaymentCompletableFuture {
     ...
     * 这意味着，在Future版本中，查询、支付和发送邮件操作是分阶段进行的。在每个阶段，操作会并发执行，
     * 但下一个阶段的操作需要等待上一个阶段的所有操作完成才能开始。
     *
     * 与之相比，CompletableFuture版本允许同时并发处理多个订单的查询、支付和发送邮件操作，这意味着，
     * 在处理一个订单的makePayment操作时，可能同时处理另一个订单的sendEmail操作。
     */
