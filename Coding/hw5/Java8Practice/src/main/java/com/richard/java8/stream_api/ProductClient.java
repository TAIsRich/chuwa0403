...
public class ProductClient {
    ...
    public void groupingBy() {
        ...
        categoryAveragePrice.entrySet().stream().forEach(System.out::println);
    }
    ...
    public void partitionBy() {
        ...
        Map<Boolean, List<Product>> partitionProducts = productList.stream()
                .collect(Collectors.partitioningBy(p -> p.getPrice() > 100));

        partitionProducts.entrySet().stream().forEach(System.out::println);
    }

    @Test
    public void mapping() {
        Set<String> upperCaseNames = productList.stream()
                .map(Product::getName)              // 把 product -> name
                .map(String::toUpperCase)           // 把 lowercase -> uppercase
                .collect(Collectors.toSet());
    ...
    public void match() {
        ...
        // noneMatch example
        boolean allProductsAreNotFree = productList.stream()
                .noneMatch(product -> product.getPrice() == 0);
        System.out.println("Are there no free products? " + allProductsAreNotFree);
