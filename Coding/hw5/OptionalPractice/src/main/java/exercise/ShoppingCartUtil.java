...
public class ShoppingCartUtil {
     ...
     *
     * filter(obj -> !obj.isEmpty() 常被用来判断obj是否为null
     */
     ...
     *
     * orElse 是直接返回一个对象
     * 常用办法来把一个Collection中的所有值给加起来：
     * collection.stream().mapToADataType()
    ...
    /**
     * 假设我们想要获取客户购物车中的第一个商品名称，
     * 如果不存在，则从一个备选商品列表中随机选择一个商品名称作为默认值。
     *
     * orElseGet 是调用一个可以返回一个对象的方法
     */
    ...
    /**
     * 假设我们想要获取客户购物车中的第一个商品名称，
     * 如果购物车为空或不存在，抛出一个自定义异常EmptyCartException。
     *
    ...
    public static String getFirstItemNameOrThrowException(Customer customer) throws EmptyCartException {
        return Optional.ofNullable(customer)
                ...
                .map(items -> items.get(0))
                .map(Item::getName)
                .orElseThrow(() -> new EmptyCartException("购物车不存在"));
    }

    /**
     * 假设我们想要检查客户购物车中是否有商品，并打印相应的消息。
     *
     * 编写一个方法，接受一个Customer对象作为参数，
     * 然后使用isPresent()检查购物车中是否有商品，并打印相应的消息：
     *
     * 我们使用了isPresent()方法来检查Optional对象是否非空。
     * 我们首先从Customer对象中获取购物车商品列表，然后检查其是否为空。
