...
class AddStudentThread extends Thread {
    ...
    public void run() {
        for (int i = 0; i < 10000; i++) {
            System.out.println("Its add student waiting for a lock");
            synchronized (Counter.LOCK_STUDENT) {
                Counter.studentCount += 1;
            }
            System.out.println("Its add student " + Counter.studentCount);
        }
    }
}
...
class DecStudentThread extends Thread {
    ...
    public void run() {
        for (int i = 0; i < 10000; i++) {
            System.out.println("Its dec student waiting for a lock");
            synchronized (Counter.LOCK_STUDENT) {
                Counter.studentCount -= 1;
            }
            System.out.println("Its dec student " + Counter.studentCount);
        }
    }
}
